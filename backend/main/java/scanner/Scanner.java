...
public class Scanner {
    ...
    private static String artworkPath =
        "/home/tejas/Projects/OurMusic/backend/main/resources/artwork";
    ...
    public static List<Song> scanLibrary() {
        ...
        for (File file : files) {
            try {
                ...
                String title =
                    tag != null ? tag.getFirst(FieldKey.TITLE) : file.getName();
                String[] artists;
                if (tag != null) {
                ...
                String album =
                    tag != null
                        ...
                        : "Unknown Album";
                int albumId = Database.getAlbum(
                    album,
                    Database.getArtist(artists[0].trim())
                );
                ...
                try (
                    FileOutputStream fileOutputStream = new FileOutputStream(
                        artworkPath +
                            File.separator +
                            Utils.sha256(title).substring(0, 16) +
                            ".jpg"
                    )
                ) {
                    Artwork artwork =
                        tag != null ? tag.getFirstArtwork() : null;
                    if (artwork != null) {
                ...
                songs.add(song);
                for (String artist : artists) {
                    Database.insertSongArtists(
                        song.getId(),
                        Database.getArtist(artist)
                    );
                }
            } catch (Exception e) {
